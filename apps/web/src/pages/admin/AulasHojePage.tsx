...
import { Clock, MapPin, User as UserIcon, CheckCircle2, Circle, Users } from 'lucide-react'
...
export function AulasHojePage() {
  ...
  const aulasHoje = (grade?.[diaSemanaHoje] ?? []).sort((a, b) => a.horarioInicio.localeCompare(b.horarioInicio))

  return (
    <div>
      <PageHeader
        titulo="Aulas de hoje"
        descricao={`${new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: '2-digit' })} · ${aulasHoje.length} aula(s) agendada(s).`}
      />
              ...
              <div className="flex items-start justify-between gap-2 flex-wrap">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-600 mt-1 flex flex-wrap gap-x-3 gap-y-1">
                    <span className="flex items-center gap-1"><UserIcon className="h-3 w-3" /> {a.professor.nome}</span>
                    <span className="flex items-center gap-1"><MapPin className="h-3 w-3" /> {a.sala.nome}</span>
                  </div>
                </div>
                <ContagemReservas aulaId={a.id} dataAula={hoje} capacidade={a.capacidade} />
              </div>
            </button>
          ))}
        </div>
      ...
      {aulaAberta && (
        <ListaPresencaModal aula={aulaAberta} dataAula={hoje} onClose={() => setAulaAberta(null)} />
      )}
    </div>
  )
}
